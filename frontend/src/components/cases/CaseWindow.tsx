...
export function CaseWindow({ win }: CaseWindowProps) {
  ...
  React.useEffect(() => {
    ...
    let stuckSafetyTimeout: ReturnType<typeof setTimeout>;
    ...
    if (realProgress < 90) {
      ...
      // Auto-transition to graph tab and clear progress after displaying 100%
      const timer = setTimeout(() => {
        setWindows((prev: WindowItem[]) =>
      ...
      // Safety timeout: if scan background job finished or WS message was missed, auto-advance to 100%
      stuckSafetyTimeout = setTimeout(() => {
        setDisplayProgress(100);
        setTimeout(() => {
          setWindows((prev: WindowItem[]) =>
            prev.map((w) => (w.caseId === caseId ? { ...w, activeTab: 'graph' } : w))
    ...
    return () => {
      ...
      if (stuckSafetyTimeout) clearTimeout(stuckSafetyTimeout);
    };
  }, [caseIngestProgress, caseId, setWindows, loadGraphForCase]);
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2 border-b border-white/10 pb-2 mb-3 flex-shrink-0 text-[9px] font-bold">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded transition-all duration-200 uppercase whitespace-nowrap ${tab === tabItem.id
