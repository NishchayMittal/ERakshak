    ...
    <div className="flex flex-col gap-3 h-full overflow-y-auto pr-1 select-none text-gray-200">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-2">
        <span className="text-[10px] font-bold text-[#a855f7] uppercase tracking-wider flex items-center gap-1.5">
          <Network size={12} /> CROSS-CASE INTELLIGENCE CORRELATOR
        </span>
        <button
      ...
      {data && (
        <div className="flex gap-4 text-[8.5px] font-mono uppercase tracking-wider bg-black/30 p-2 rounded border border-white/5">
          <span className="text-gray-400">CASES ANALYZED: <span className="text-[#39ff14] font-bold">{data.cases_analyzed}</span></span>
          <span className="text-gray-400">SHARED IDENTIFIERS: <span className={data.total_shared_identifiers > 0 ? 'text-[#a855f7] font-bold' : 'text-gray-400'}>{data.total_shared_identifiers}</span></span>
        </div>
      )}
        ...
        <div className="flex-1 flex items-center justify-center py-12">
          <div className="text-center flex flex-col items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-[#a855f7] animate-pulse uppercase tracking-widest">
              SCANNING IDENTIFIER OVERLAP ACROSS ALL DOSSIERS...
            </span>
          </div>
        </div>
      )}
      ...
      {!loading && data && data.correlations.length === 0 && (
        <div className="flex-1 flex items-center justify-center py-12">
          <div className="text-center flex flex-col items-center gap-2">
            ...
            <p className="text-[10px] font-mono text-gray-400 uppercase font-bold">NO SHARED IDENTIFIERS DETECTED</p>
            <p className="text-[8.5px] font-mono text-gray-500 max-w-xs">
              Identifiers (emails, phones, IPs, usernames) must appear across 2 or more cases to trigger cross-correlation alerts.
            </p>
          </div>
        </div>
      )}
          ...
          {data.correlations.map((corr, idx) => (
            <div key={idx} className="bg-[#a855f7]/5 border border-[#a855f7]/20 p-3 rounded-lg hover:border-[#a855f7]/40 transition-all">
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[8px] text-gray-400 font-mono uppercase font-bold">APPEARS IN {corr.case_count} CASES:</span>
